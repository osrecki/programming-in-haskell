...
Description : Solutions to Homework 1 exercises
...
{-
  1.1
  - Write a function which checks if a year is a leap year.
-}
isLeapYear :: Int -> Bool
...
{-
  ...
  2 a
  - Write a function which evaluates polynomial of single indeterminante 'x'
    where polynomial coefficients are represented by a list of Doubles.
-}
...
{-
  2 b
  - Define a function which calculates the nth factorial.
-}
...
{-
  2 d
  - Combine evaluate, factorial and maclaurin to create a function which
    approximates exp from the first 170 terms of the series.
-}
exp' :: Double -> Double
...
{-
  3
  - Implement some polymorphic utility functions for working on lists of key
    value pairs where keys are strings.

  3 a
  - Implement a function for getting a pair with a certain key which should
    return a list with a single element as a result (or no elements if key
    doesn’t exist).
-}
...
{-
  3 b
  - Implement a function which checks if a list contains an element with a
    certain key.
-}
...
{-
  3 c
  - Implement a function which tries to retrieve a value with a certain key or
...
{-
  3 d
  - Implement a function which inserts a new key value pair. If the key already
...
{-
  3 e
  - Implement a function which removes a key value pair with the certain key.
-}
...
{-
  3 f
  - Implement a function which updates the value of a certain key (if the key
    does not exist, function does nothing).
-}
...
{-
  4
  - Implement a function which calculates a cosine similarity between two texts.
-}
