...
Description : Solutions to Homework 3 exercises
...
{-
  1
  - A local maximum in a list is an element which is strictly greater than both
    the elements immediately before and after it. Define a function which finds
    all local maxima in the list and returns them in order.
-}
localMaxima :: [Int] -> [Int]
...
{-
  3
  - Implement a simplified version of the cellular automaton, which works on a
    fixed-size array, and only the Rule 90:
    <https://en.wikipedia.org/wiki/Rule_90>

  - Example for the Sierpinski triangle:
